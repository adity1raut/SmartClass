function QuestionCard({
  ...
  return (
    <div
      className="sc-card-premium glass rounded-2xl p-6 mb-5 animate-[slide-up_0.5s_cubic-bezier(0.16,1,0.3,1)_both]"
...
function OptionButton({ option, optionIndex, isSelected, onClick, disabled }) {
  return (
    ...
    >
      ...
      <span
        className={`text-sm font-medium transition-colors duration-300 ${isSelected ? "text-[var(--accent)]" : "text-[var(--text)]"}`}
      >
        {option}
      </span>
      {isSelected && (
        <span className="ml-auto text-[var(--accent)] text-sm animate-[scale-in_0.3s_ease_both]">
          ✓
        </span>
      )}
    </div>
