...
const scoreColor = (pct) =>
  pct >= 70 ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/25"
  : pct >= 40 ? "bg-amber-500/15 text-amber-400 border-amber-500/25"
  : "bg-red-500/15 text-red-400 border-red-500/25";

function AssignmentCard({ assignment, isTeacher, mySubmission, submissions, submissionText, onSubmit, onToggleSubs, onDelete, onGrade }) {
  ...
  return (
    <div className="group sc-card-premium glass rounded-2xl border border-[var(--border)]/20 hover:border-[var(--accent)]/20 transition-all duration-300 overflow-hidden">
      ...
      <div className={`h-1 w-full ${
        isOverdue && !hasSubmission ? "bg-gradient-to-r from-red-500 to-red-600"
        : hasSubmission ? "bg-gradient-to-r from-emerald-500 to-teal-500"
      ...
      <div className="p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3 mb-2">
              <p className="text-sm font-bold text-[var(--text)] leading-snug">{assignment.title}</p>
              <div className="flex items-center gap-2 shrink-0">
                ...
                {isTeacher ? (
                  ...
                  <span className={`px-3 py-1.5 rounded-xl text-xs font-black border ${scoreColor(scorePct(mySubmission.score, assignment.maxScore))}`}>
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {assignment.dueDate && (
                <span className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold border
                  ${isOverdue
                    ? "bg-red-500/12 text-red-400 border-red-500/20"
                    : "bg-[var(--border)]/15 text-[var(--muted)] border-[var(--border)]/20"
                  }`}>
                  {isOverdue ? "⚠ Overdue" : "📅"} {new Date(assignment.dueDate).toLocaleDateString()}
                </span>
              )}
            ...
            {assignment.description && (
              <p className="text-xs text-[var(--muted)] leading-relaxed">{assignment.description}</p>
            )}
          </div>
        </div>
        ...
        {!isTeacher && !hasSubmission && (
          <div className="mt-4 pt-4 border-t border-[var(--border)]/15">
            <p className="text-xs font-bold text-[var(--muted)] mb-2 uppercase tracking-wider">Your Answer</p>
            <textarea
        ...
        {!isTeacher && hasSubmission && (
          <div className="mt-4 pt-4 border-t border-[var(--border)]/15">
            <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Your Submission</p>
            <div className="glass rounded-xl px-4 py-3 border border-[var(--border)]/15">
              <p className="text-sm text-[var(--text)] leading-relaxed">{mySubmission.content}</p>
              {mySubmission.feedback && (
                <div className="mt-3 pt-3 border-t border-[var(--border)]/15">
                  <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider mb-1">Teacher Feedback</p>
                  <p className="text-sm text-[var(--text)] leading-relaxed italic">"{mySubmission.feedback}"</p>
                </div>
              )}
            </div>
          </div>
        ...
        {isTeacher && submissions !== undefined && (
          <div className="mt-4 pt-4 border-t border-[var(--border)]/15">
            {submissions.length === 0 ? (
              <p className="text-xs text-[var(--muted)] text-center py-4">No submissions yet.</p>
            ) : (
              <div className="space-y-2">
                {submissions.map(s => {
                  const pct = s.status === "graded" ? scorePct(s.score, assignment.maxScore) : null;
                  return (
                    <div key={s.id} className="glass rounded-xl px-4 py-3 border border-[var(--border)]/15">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-xs font-bold text-[var(--text)]">{s.student?.name}</p>
                            <p className="text-[10px] text-[var(--muted)]">
                              {new Date(s.submittedAt).toLocaleString()}
                              {s.status === "late" && <span className="ml-2 text-red-400">⏰ Late</span>}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          {pct !== null && (
                            <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black border ${scoreColor(pct)}`}>
                      ...
                      <p className="text-xs text-[var(--text)] leading-relaxed pl-9">{s.content}</p>
                      {s.feedback && (
                        <p className="text-[10px] text-[var(--muted)] mt-1 pl-9 italic">"{s.feedback}"</p>
                      )}
                    </div>
                  );
                })}
