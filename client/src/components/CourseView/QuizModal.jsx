...
function QuizModal({ isOpen, form, saving, onSubmit, onClose, onChange }) {
  ...
  const handleUpdateQuestion = (qi, field, val) => {
    onChange({
      ...form,
      questions: form.questions.map((q, i) => i === qi ? { ...q, [field]: val } : q)
    });
  };
  ...
  const handleAddQuestion = () => {
    onChange({
      ...form,
      questions: [...form.questions, { question: "", options: ["", "", "", ""], answer: 0 }]
    });
  };
  ...
  return modalOverlay(onClose, <>
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-[var(--muted)] mb-1.5">Time Limit (min, 0=none)</label>
          <input type="number" min={0} className={inputCls} value={form.timeLimit}
        ...
        <div key={qi} className="border border-[var(--border)] rounded-xl p-4 bg-[var(--bg)]">
          <div className="flex items-center justify-between mb-3">
            ...
            {form.questions.length > 1 && (
              <button type="button" onClick={() => handleRemoveQuestion(qi)}
                className="text-red-400 hover:text-red-600 bg-transparent border-none cursor-pointer text-lg leading-none">×</button>
            )}
          </div>
          <input className={`${inputCls} mb-3`} placeholder="Question text..."
            value={q.question} onChange={e => handleUpdateQuestion(qi, "question", e.target.value)} required />
          {q.options.map((opt, oi) => (
            <div key={oi} className="flex items-center gap-2 mb-2">
              <input type="radio" name={`q${qi}`} checked={q.answer === oi}
                onChange={() => handleUpdateQuestion(qi, "answer", oi)}
                className="cursor-pointer accent-[var(--accent)]" title="Mark as correct answer" />
              <input
                ...
                value={opt} onChange={e => handleUpdateOption(qi, oi, e.target.value)} required />
            </div>
          ))}
          <p className="text-xs text-[var(--muted)] mt-1">Click the radio to mark the correct answer</p>
        </div>
      ))}
