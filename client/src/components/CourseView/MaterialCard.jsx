...
const TYPE_META = {
  video:    { icon: "▶", label: "Video",    bg: "from-red-500/20 to-red-600/10",    text: "text-red-400",    border: "border-red-500/20" },
  document: { icon: "📄", label: "Document", bg: "from-amber-500/20 to-amber-600/10", text: "text-amber-400",  border: "border-amber-500/20" },
  link:     { icon: "🔗", label: "Link",     bg: "from-blue-500/20 to-blue-600/10",   text: "text-blue-400",   border: "border-blue-500/20" },
  image:    { icon: "🖼", label: "Image",    bg: "from-purple-500/20 to-purple-600/10", text: "text-purple-400", border: "border-purple-500/20" },
};
const defaultMeta = { icon: "📝", label: "Other", bg: "from-emerald-500/20 to-emerald-600/10", text: "text-emerald-400", border: "border-emerald-500/20" };

function MaterialCard({ material, isTeacher, isDone, onToggleComplete, onDelete }) {
  const meta = TYPE_META[material.type] || defaultMeta;

  return (
    <div className={`group sc-card-premium glass rounded-2xl p-5 border transition-all duration-300
                     ${isDone
      ...
      <div className="flex items-start gap-4">
        {/* Type icon */}
        <div className={`w-11 h-11 rounded-2xl bg-gradient-to-br ${meta.bg} border ${meta.border}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3 mb-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-sm font-bold text-[var(--text)]">{material.title}</span>
          ...
          {material.description && (
            <p className="text-xs text-[var(--muted)] mb-2 leading-relaxed">{material.description}</p>
          )}
      ...
      {isDone && (
        <div className="mt-3 pt-3 border-t border-emerald-500/15 flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">Material completed</span>
        </div>
      )}
    </div>
  );
