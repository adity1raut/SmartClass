function LiveClassCard({ liveClass, isTeacher, onStatusChange, onDelete, onJoin }) {
  ...
  const statusMeta = isLive
    ? { label: "🔴 LIVE NOW", bg: "bg-red-500/15", text: "text-red-400", border: "border-red-500/30", glow: "shadow-[0_0_20px_-4px_rgba(239,68,68,0.35)]", bar: "from-red-500 to-red-600" }
    : isEnded
    ? { label: "⬛ Ended",    bg: "bg-[var(--border)]/10", text: "text-[var(--muted)]", border: "border-[var(--border)]/20", glow: "", bar: "from-gray-500 to-gray-600" }
    : { label: "🗓 Scheduled", bg: "bg-blue-500/12",  text: "text-blue-400",  border: "border-blue-500/25",  glow: "", bar: "from-blue-500 to-indigo-600" };

  return (
    <div className={`group sc-card-premium glass rounded-2xl border overflow-hidden transition-all duration-300
                     ${isLive ? "border-red-500/30 shadow-[0_0_24px_-6px_rgba(239,68,68,0.25)]" : "border-[var(--border)]/20 hover:border-[var(--accent)]/20"}`}>
      {/* Accent bar */}
      <div className={`h-1 w-full bg-gradient-to-r ${statusMeta.bar} ${isLive ? "animate-pulse" : ""}`} />
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3 mb-2">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <p className="text-sm font-bold text-[var(--text)]">{liveClass.title}</p>
                <span className={`px-2.5 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider border ${statusMeta.bg} ${statusMeta.text} ${statusMeta.border}`}>
                  {statusMeta.label}
                </span>
              </div>
              {liveClass.description && (
                <p className="text-xs text-[var(--muted)] mb-2 leading-relaxed">{liveClass.description}</p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-bold bg-[var(--border)]/12 text-[var(--muted)] border border-[var(--border)]/15">
              📅 {new Date(liveClass.scheduledAt).toLocaleDateString()} · {new Date(liveClass.scheduledAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
            </span>
