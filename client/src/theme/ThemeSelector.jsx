...
const THEME_META = {
  ...
  [themes.custom]: { label: "Custom", icon: "✨" },
};

function ThemeSelector({ themeName, setThemeName }) {
  return (
    <div className="fixed right-3 top-3 z-50 rounded-2xl border border-[var(--border)] bg-[color-mix(in_srgb,var(--surface)_82%,transparent)] backdrop-blur-xl px-2 py-2 shadow-[0_10px_30px_rgba(0,0,0,0.18)]">
            ...
            <button
              ...
              className={`group relative flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-semibold transition-all duration-200 ${
                active
                  ...
                  : "bg-[var(--surface)] text-[var(--muted)] hover:text-[var(--text)] hover:-translate-y-0.5"
              }`}
              type="button"
