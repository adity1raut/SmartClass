...
function AiResultView({ data }) {
  ...
  if (data.questions && Array.isArray(data.questions)) {
    return (
      <div className="space-y-3">
        <p className="text-xs font-semibold text-[var(--muted)] uppercase tracking-wider">{data.count} questions · {data.topic} · {data.difficulty}</p>
        {data.questions.map((q, i) => (
          <div key={i} className="border border-[var(--border)]/40 rounded-xl p-3 space-y-2">
            <p className="font-semibold text-sm text-[var(--text)]"><span className="text-[var(--accent)] mr-1.5">Q{i + 1}.</span>{q.question}</p>
            <ul className="space-y-1">
              {q.options.map((opt, j) => (
                <li key={j} className={`text-xs px-2.5 py-1.5 rounded-lg ${j === q.correct_answer ? "bg-green-500/15 text-green-400 font-semibold border border-green-500/30" : "text-[var(--muted)]"}`}>
                  {String.fromCharCode(65 + j)}. {opt}
                </li>
              ))}
            </ul>
            {q.explanation && <p className="text-xs text-[var(--muted)] bg-[var(--border)]/10 rounded-lg px-2.5 py-1.5">💡 {q.explanation}</p>}
          </div>
        ))}
      </div>
    );
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap gap-1.5">
          {data.tools_used.map((t, i) => (
            <span key={i} className={`text-xs px-2.5 py-1 rounded-full font-semibold border ${t.success ? "border-green-500/30 text-green-400 bg-green-500/10" : "border-red-400/30 text-red-400 bg-red-500/10"}`}>
              {t.success ? "✓" : "✗"} {t.tool}
            </span>
          ))}
        </div>
  ...
  const text = data.outline || data.feedback || data.response || data.summary || data.explanation || data.study_plan || data.analysis;
  if (text) return <AiMarkdown text={text} />;
  return <pre className="text-xs text-[var(--text)] whitespace-pre-wrap font-mono">{JSON.stringify(data, null, 2)}</pre>;
}
...
function TeacherDashboard() {
  ...
  const [quizForm, setQuizForm] = useState({ topic: "", num_questions: 5, difficulty: "medium", content: "" });
  const [outlineForm, setOutlineForm] = useState({ course_title: "", subject: "", duration_weeks: 8, target_level: "intermediate", learning_objectives: "" });
  ...
  const [manualQs, setManualQs] = useState([{ question: "", options: ["", "", "", ""], correct_answer: 0 }]);
  ...
  const openAiModal = (type, course = null) => {
    ...
    if (type === "outline" && course) {
      setOutlineForm((f) => ({ ...f, course_title: course.title, subject: course.subject || "" }));
    }
    if (type === "outline") {
  ...
  const closeAiModal = () => {
    setAiModal(null); setAiResult(null); setAiError("");
    setEditableQuestions([]); setQuizSaved(null); setOutlineSaved(null);
  };
  ...
  const saveQuiz = async () => {
    ...
    if (!courseId) { setAiError("Select a course to save this quiz to."); return; }
    if (!quizTitle.trim()) { setAiError("Quiz title is required."); return; }
    setQuizSaving(true); setAiError("");
    try {
  ...
  const saveOutline = async () => {
    setOutlineSaving(true); setAiError("");
    try {
  ...
  const loadContent = async (courseId) => {
    if (!courseId) { setCourseQuizzes([]); setCourseAssignments([]); return; }
  ...
  const toggleQuizActive = async (quiz) => {
    try {
      ...
      if (res.ok) setCourseQuizzes((prev) => prev.map((q) => q.id === quiz.id ? { ...q, isActive: updated.isActive } : q));
    } catch {/* silent */}
  };
  ...
  const createManualQuiz = async () => {
    ...
    if (manualQs.some((q) => !q.question.trim() || q.options.some((o) => !o.trim()))) {
      alert("Fill in all question texts and options."); return;
    }
    ...
    try {
      const res = await fetch(`/api/courses/${contentCourse}/quizzes`, {
        ...
        body: JSON.stringify({
          title: manualTitle, teacherId: user.id,
          questions: manualQs.map((q) => ({ question: q.question, options: q.options, correctOption: q.correct_answer, points: 1 })),
        }),
      });
      const newQuiz = await res.json();
      if (res.ok) {
        setCourseQuizzes((prev) => [newQuiz, ...prev]);
        setShowQuizBuilder(false); setManualTitle("");
        setManualQs([{ question: "", options: ["", "", "", ""], correct_answer: 0 }]);
        load();
      }
    } catch {/* silent */} finally { setQuizBuilding(false); }
  };
  ...
  const createAssignmentInCourse = async () => {
    ...
    try {
      const res = await fetch(`/api/courses/${contentCourse}/assignments`, {
        method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include",
        body: JSON.stringify({ ...assignForm, maxScore: Number(assignForm.maxScore), teacherId: user.id }),
      });
    ...
    } catch {/* silent */} finally { setAssignBuilding(false); }
  };

  const generateAssignDesc = async () => {
    const course = data.courses.find((c) => c.id === contentCourse);
    if (!assignForm.title || !contentCourse) { alert("Enter a title first."); return; }
    setAssignAiLoading(true);
    try {
      const res = await fetch("/api/ai/generate-assignment", {
        method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include",
        body: JSON.stringify({ course_title: course?.title || "", topic: assignForm.title }),
      });
      const d = await res.json();
      if (d.response) setAssignForm((f) => ({ ...f, description: d.response }));
    } catch {/* silent */} finally { setAssignAiLoading(false); }
  };

  const updateManualQ = (qi, field, val) =>
    setManualQs((prev) => prev.map((q, i) => i === qi ? { ...q, [field]: val } : q));
  ...
  const updateQuestion = (qi, field, value) => {
    setEditableQuestions((prev) => prev.map((q, i) => i === qi ? { ...q, [field]: value } : q));
  };
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-8 z-10">
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { icon: "📝", label: "Generate Quiz", desc: "Create MCQ questions for any topic with AI", action: () => openAiModal("quiz"), color: "from-blue-500/15 to-blue-600/5", accent: "blue-500" },
              { icon: "🎓", label: "Course Outline", desc: "Generate a full week-by-week course plan", action: () => openAiModal("outline"), color: "from-purple-500/15 to-purple-600/5", accent: "purple-500" },
              { icon: "⚡", label: "AI Agent", desc: "Multi-step tasks — quiz + outline + more", action: () => openAiModal("agent"), color: "from-violet-500/15 to-violet-600/5", accent: "violet-500" },
            ].map((tool) => (
              <button
                key={tool.label}
                onClick={tool.action}
              ...
              >
                ...
                <p className="text-sm font-extrabold text-[var(--text)] mb-1">{tool.label}</p>
                <p className="text-xs text-[var(--muted)] font-medium leading-relaxed">{tool.desc}</p>
              </button>
            ))}
          </div>
        </div>
              ...
              >
                ...
                <div className="relative z-10 flex flex-col h-full">
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); openAiModal("quiz", c); }}
        ...
        {data.courses.length > 0 && (
          <div className="mt-12 animate-[slide-up_0.6s_cubic-bezier(0.16,1,0.3,1)_both]" style={{ animationDelay: "450ms" }}>
            <h2 className="text-2xl font-extrabold text-[var(--text)] mb-5 flex items-center gap-3 sc-title">
            ...
            <div className="flex items-center gap-3 mb-6">
              <select
                ...
                value={contentCourse}
                onChange={(e) => { setContentCourse(e.target.value); setManageTab("quizzes"); setShowQuizBuilder(false); setShowAssignBuilder(false); loadContent(e.target.value); }}
              >
              ...
              {contentCourse && (
                <div className="flex gap-2">
                  <button onClick={() => openAiModal("quiz", data.courses.find(c => c.id === contentCourse))} className="px-4 py-2.5 rounded-xl border border-violet-500/30 text-violet-400 text-sm font-bold hover:bg-violet-500/10 transition-all cursor-pointer flex items-center gap-2">🤖 AI Quiz</button>
                  <button onClick={() => openAiModal("class-agenda", data.courses.find(c => c.id === contentCourse))} className="px-4 py-2.5 rounded-xl border border-indigo-500/30 text-indigo-400 text-sm font-bold hover:bg-indigo-500/10 transition-all cursor-pointer flex items-center gap-2">📹 Class Agenda</button>
                </div>
              )}
            </div>
              ...
              <div className="sc-card-premium glass rounded-2xl overflow-hidden border border-[var(--border)]/30">
                ...
                <div className="flex border-b border-[var(--border)]/30">
                  {["quizzes", "assignments"].map((tab) => (
                    <button key={tab} onClick={() => setManageTab(tab)} className={`flex-1 py-3.5 text-sm font-bold capitalize transition-all cursor-pointer ${manageTab === tab ? "bg-[var(--accent)]/10 text-[var(--accent)] border-b-2 border-[var(--accent)]" : "text-[var(--muted)] hover:text-[var(--text)]"}`}>
                      {tab === "quizzes" ? `🧠 Quizzes (${courseQuizzes.length})` : `📝 Assignments (${courseAssignments.length})`}
                    </button>
                  ))}
                </div>
                ...
                <div className="p-5">
                  {contentLoading ? (
                    <div className="flex items-center justify-center py-12 text-[var(--muted)]">
                      <span className="w-6 h-6 border-2 border-current/30 border-t-current rounded-full animate-spin mr-3" />Loading…
                    </div>
                  ) : (
                    <>
                      {/* ── QUIZZES ── */}
                      {manageTab === "quizzes" && (
                        <div className="space-y-4">
                          <div className="flex gap-2">
                            <button onClick={() => { setShowQuizBuilder(!showQuizBuilder); setShowAssignBuilder(false); }} className="px-4 py-2.5 sc-btn-glow rounded-xl text-sm font-bold cursor-pointer active:scale-95 flex items-center gap-2">
                              {showQuizBuilder ? "✕ Cancel" : "+ Create Manually"}
                            </button>
                          </div>
                          ...
                          {showQuizBuilder && (
                            <div className="border border-[var(--border)]/40 rounded-xl p-4 space-y-4 glass">
                              <input className={inputCls} placeholder="Quiz Title *" value={manualTitle} onChange={(e) => setManualTitle(e.target.value)} />
                              <div className="space-y-4 max-h-[50vh] overflow-y-auto pr-1">
                                {manualQs.map((q, qi) => (
                                  <div key={qi} className="border border-[var(--border)]/30 rounded-xl p-3 space-y-2">
                                    <div className="flex items-start gap-2">
                                      <span className="w-6 h-6 rounded-lg bg-[var(--accent)]/15 flex items-center justify-center text-xs font-bold text-[var(--accent)] shrink-0 mt-2">Q{qi+1}</span>
                                      <textarea className={`${inputCls} resize-none flex-1`} rows={2} placeholder="Question text" value={q.question} onChange={(e) => updateManualQ(qi, "question", e.target.value)} />
                                      {manualQs.length > 1 && <button onClick={() => setManualQs((p) => p.filter((_, i) => i !== qi))} className="text-red-400 text-xs hover:text-red-300 cursor-pointer px-2 shrink-0 mt-2">✕</button>}
                                    </div>
                                    <div className="space-y-1.5 pl-8">
                                      {q.options.map((opt, oi) => (
                                        <div key={oi} className="flex items-center gap-2">
                                          <input type="radio" name={`mq-correct-${qi}`} checked={q.correct_answer === oi} onChange={() => updateManualQ(qi, "correct_answer", oi)} className="w-4 h-4 accent-[var(--accent)] cursor-pointer shrink-0" title="Correct answer" />
                                          <input className={`${inputCls} text-sm py-2 ${q.correct_answer === oi ? "border-green-500/50 bg-green-500/5" : ""}`} placeholder={`Option ${String.fromCharCode(65+oi)}`} value={opt} onChange={(e) => updateManualOpt(qi, oi, e.target.value)} />
                                          {q.correct_answer === oi && <span className="text-green-400 text-xs shrink-0">✓</span>}
                                        </div>
                                      ))}
                                      <p className="text-[10px] text-[var(--muted)]">Click radio → set correct answer</p>
                                    </div>
                                  </div>
                                ))}
                              </div>
                              <div className="flex gap-2">
                                <button onClick={() => setManualQs((p) => [...p, { question: "", options: ["", "", "", ""], correct_answer: 0 }])} className="px-4 py-2 glass border border-[var(--border)]/40 rounded-xl text-sm font-bold cursor-pointer hover:bg-[var(--accent)]/10 transition-all">+ Add Question</button>
                                <button onClick={createManualQuiz} disabled={quizBuilding} className="flex-1 py-2 sc-btn-glow rounded-xl text-sm font-bold cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2">
                                  {quizBuilding ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving…</> : "💾 Save Quiz"}
                                </button>
                              </div>
                            </div>
                          )}
                          ...
                          {courseQuizzes.length === 0 ? (
                            <p className="text-sm text-[var(--muted)] text-center py-8">No quizzes yet. Create one manually or with AI.</p>
                          ) : (
                            <div className="space-y-2">
                              {courseQuizzes.map((q) => (
                                <div key={q.id} className="flex items-center justify-between gap-3 px-4 py-3 rounded-xl border border-[var(--border)]/30 glass group">
                                  <div className="flex-1 min-w-0">
                                    <p className="text-sm font-bold text-[var(--text)] truncate">{q.title}</p>
                                    <p className="text-xs text-[var(--muted)]">{q.questionCount ?? q.questions?.length ?? 0} questions · {new Date(q.createdAt).toLocaleDateString()}</p>
                                  </div>
                                  <div className="flex items-center gap-2 shrink-0">
                                    <button onClick={() => toggleQuizActive(q)} className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all cursor-pointer ${q.isActive ? "bg-green-500/15 text-green-400 border-green-500/30 hover:bg-green-500/25" : "bg-[var(--border)]/20 text-[var(--muted)] border-[var(--border)]/30 hover:border-[var(--accent)]/30"}`}>
                                      {q.isActive ? "● Published" : "○ Draft"}
                                    </button>
                                    <button onClick={() => deleteQuizFromCourse(q.id)} className="p-1.5 rounded-lg text-[var(--muted)] hover:text-red-400 hover:bg-red-500/10 transition-all cursor-pointer opacity-0 group-hover:opacity-100 text-xs">🗑</button>
                                  </div>
                                </div>
                              ))}
                            </div>
                      ...
                      {manageTab === "assignments" && (
                        <div className="space-y-4">
                          <button onClick={() => { setShowAssignBuilder(!showAssignBuilder); setShowQuizBuilder(false); }} className="px-4 py-2.5 sc-btn-glow rounded-xl text-sm font-bold cursor-pointer active:scale-95">
                            {showAssignBuilder ? "✕ Cancel" : "+ Create Assignment"}
                          </button>

                          {/* Assignment builder */}
                          {showAssignBuilder && (
                            <div className="border border-[var(--border)]/40 rounded-xl p-4 space-y-3 glass">
                              <input className={inputCls} placeholder="Assignment Title *" value={assignForm.title} onChange={(e) => setAssignForm((f) => ({ ...f, title: e.target.value }))} />
                              <div className="relative">
                                <textarea className={`${inputCls} resize-none`} rows={4} placeholder="Description (or use AI to generate)" value={assignForm.description} onChange={(e) => setAssignForm((f) => ({ ...f, description: e.target.value }))} />
                                <button onClick={generateAssignDesc} disabled={assignAiLoading} className="absolute bottom-2 right-2 px-3 py-1.5 rounded-lg bg-violet-500/15 text-violet-400 text-xs font-bold border border-violet-500/20 hover:bg-violet-500/25 transition-all cursor-pointer disabled:opacity-50 flex items-center gap-1.5">
                                  {assignAiLoading ? <span className="w-3 h-3 border border-current/30 border-t-current rounded-full animate-spin" /> : "🤖"} Generate
                                </button>
                              </div>
                              <div className="grid grid-cols-2 gap-3">
                                <div>
                                  <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-1.5">Due Date</label>
                                  <input type="datetime-local" className={inputCls} value={assignForm.dueDate} onChange={(e) => setAssignForm((f) => ({ ...f, dueDate: e.target.value }))} />
                                </div>
                                <div>
                                  <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-1.5">Max Score</label>
                                  <input type="number" min={1} max={1000} className={inputCls} value={assignForm.maxScore} onChange={(e) => setAssignForm((f) => ({ ...f, maxScore: e.target.value }))} />
                                </div>
                              </div>
                              <button onClick={createAssignmentInCourse} disabled={assignBuilding} className="w-full py-2.5 sc-btn-glow rounded-xl text-sm font-bold cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2">
                                {assignBuilding ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving…</> : "💾 Save Assignment"}
                              </button>
                            </div>
                          )}
                          ...
                          {courseAssignments.length === 0 ? (
                            <p className="text-sm text-[var(--muted)] text-center py-8">No assignments yet. Create one above.</p>
                          ) : (
                            <div className="space-y-2">
                              {courseAssignments.map((a) => (
                                <div key={a.id} className="flex items-center justify-between gap-3 px-4 py-3 rounded-xl border border-[var(--border)]/30 glass group">
                                  <div className="flex-1 min-w-0">
                                    <p className="text-sm font-bold text-[var(--text)] truncate">{a.title}</p>
                                    <p className="text-xs text-[var(--muted)]">
                                      Max: {a.maxScore}pts
                                      {a.dueDate ? ` · Due: ${new Date(a.dueDate).toLocaleDateString()}` : " · No due date"}
                                    </p>
                                  </div>
                                  <button onClick={() => deleteAssignmentFromCourse(a.id)} className="p-1.5 rounded-lg text-[var(--muted)] hover:text-red-400 hover:bg-red-500/10 transition-all cursor-pointer opacity-0 group-hover:opacity-100 text-xs shrink-0">🗑</button>
                                </div>
                              ))}
                            </div>
                          )}
          ...
          <div className="glass-heavy border border-[var(--border)]/50 rounded-2xl w-full max-w-3xl max-h-[92vh] flex flex-col shadow-[0_32px_80px_-16px_rgba(0,0,0,0.5)] animate-[scale-in_0.3s_cubic-bezier(0.16,1,0.3,1)_both]">
            ...
            <div className="flex items-center justify-between p-5 border-b border-[var(--border)]/30 shrink-0">
              <h3 className="text-lg font-extrabold text-[var(--text)] flex items-center gap-3">
                <span className="w-9 h-9 rounded-xl bg-violet-500/15 flex items-center justify-center text-lg">🤖</span>
            ...
            <div className="overflow-y-auto flex-1 p-5 space-y-5">
              ...
              {aiModal === "quiz" && (
                <>
                  ...
                  {editableQuestions.length === 0 && !quizSaved && (
                    <form onSubmit={(e) => { e.preventDefault(); runAi("/generate-quiz", { ...quizForm, num_questions: Number(quizForm.num_questions), content: quizForm.content || undefined }); }} className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="col-span-2">
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Topic *</label>
                          <input className={inputCls} required value={quizForm.topic} onChange={(e) => setQuizForm((f) => ({ ...f, topic: e.target.value }))} placeholder="e.g. Recursion, Binary Trees…" />
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Difficulty</label>
                          <select className={inputCls} value={quizForm.difficulty} onChange={(e) => setQuizForm((f) => ({ ...f, difficulty: e.target.value }))}>
                            <option value="easy">Easy</option><option value="medium">Medium</option><option value="hard">Hard</option>
                          </select>
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Questions</label>
                          <input type="number" min={1} max={20} className={inputCls} value={quizForm.num_questions} onChange={(e) => setQuizForm((f) => ({ ...f, num_questions: e.target.value }))} />
                        </div>
                        <div className="col-span-2">
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Course Content (optional)</label>
                          <textarea className={`${inputCls} resize-none`} rows={3} placeholder="Paste material to base questions on…" value={quizForm.content} onChange={(e) => setQuizForm((f) => ({ ...f, content: e.target.value }))} />
                        </div>
                      </div>
                      <button type="submit" disabled={aiLoading} className="w-full py-3 sc-btn-glow rounded-xl text-sm font-bold cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2">
                        {aiLoading ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Generating…</> : "Generate Quiz Questions →"}
                      </button>
                    </form>
                  )}
                    ...
                    <div className="space-y-4" ref={aiResultRef}>
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-bold text-green-400 flex items-center gap-2">
                          ...
                          {editableQuestions.length} questions generated — review &amp; edit below
                        </p>
                        <button onClick={() => { setEditableQuestions([]); setAiResult(null); }} className="text-xs text-[var(--muted)] hover:text-[var(--text)] transition-colors">← Regenerate</button>
                      </div>

                      {/* Quiz title + course selector */}
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-1.5">Quiz Title *</label>
                          <input className={inputCls} value={quizTitle} onChange={(e) => setQuizTitle(e.target.value)} placeholder="e.g. Recursion Quiz — Week 3" />
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-1.5">Save to Course *</label>
                          {aiCourse ? (
                            <div className={`${inputCls} opacity-60`}>{aiCourse.title}</div>
                          ) : (
                            <select className={inputCls} value={saveTargetCourseId} onChange={(e) => setSaveTargetCourseId(e.target.value)}>
                              <option value="">— Select a course —</option>
                              {data.courses.map((c) => <option key={c.id} value={c.id}>{c.title}</option>)}
                            </select>
                          )}
                        ...
                        {editableQuestions.map((q, qi) => (
                          <div key={qi} className="border border-[var(--border)]/40 rounded-xl p-4 space-y-3 glass">
                            <div className="flex items-start gap-3">
                              <span className="w-7 h-7 rounded-lg bg-[var(--accent)]/15 flex items-center justify-center text-xs font-bold text-[var(--accent)] shrink-0 mt-0.5">Q{qi+1}</span>
                              <textarea
                            ...
                            <div className="space-y-1.5 pl-10">
                              {q.options.map((opt, oi) => (
                                <div key={oi} className="flex items-center gap-2">
                                  ...
                                  />
                                  {q.correct_answer === oi && <span className="text-green-400 text-xs shrink-0 font-bold">✓</span>}
                                </div>
                              ))}
                              <p className="text-[10px] text-[var(--muted)] pt-0.5">Click radio button to set correct answer</p>
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      >
                        {quizSaving ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving to Course…</> : "💾 Save Quiz to Course →"}
                      </button>
                    </div>
                  )}
                  ...
                  {quizSaved && (
                    <div className="text-center py-8 space-y-4">
                      ...
                      <p className="text-lg font-extrabold text-[var(--text)]">Quiz saved successfully!</p>
                      <p className="text-sm text-[var(--muted)]">
                        <strong className="text-[var(--text)]">"{quizSaved.title}"</strong> · {quizSaved.questionCount} questions added to the course
                      </p>
                      <div className="flex gap-3 justify-center">
                        <button onClick={() => navigate(`/course/${quizSaved.course}/quizzes`)} className="px-5 py-2.5 sc-btn-glow rounded-xl text-sm font-bold cursor-pointer active:scale-95">View Quiz →</button>
                        <button onClick={() => { setQuizSaved(null); setEditableQuestions([]); setAiResult(null); }} className="px-5 py-2.5 glass border border-[var(--border)]/40 rounded-xl text-sm font-bold cursor-pointer hover:bg-[var(--accent)]/10 active:scale-95">Generate Another</button>
                      </div>
                    </div>
                  )}
                </>
              ...
              {aiModal === "outline" && (
                <>
                  {!aiResult && (
                    <form onSubmit={(e) => { e.preventDefault(); runAi("/course-outline", { ...outlineForm, duration_weeks: Number(outlineForm.duration_weeks), learning_objectives: outlineForm.learning_objectives || undefined }); }} className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="col-span-2">
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Course Title *</label>
                          <input className={inputCls} required value={outlineForm.course_title} onChange={(e) => setOutlineForm((f) => ({ ...f, course_title: e.target.value }))} placeholder="e.g. Introduction to Machine Learning" />
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Subject *</label>
                          <input className={inputCls} required value={outlineForm.subject} onChange={(e) => setOutlineForm((f) => ({ ...f, subject: e.target.value }))} placeholder="e.g. Computer Science" />
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Duration (weeks)</label>
                          <input type="number" min={1} max={52} className={inputCls} value={outlineForm.duration_weeks} onChange={(e) => setOutlineForm((f) => ({ ...f, duration_weeks: e.target.value }))} />
                        </div>
                        <div className="col-span-2">
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Target Level</label>
                          <select className={inputCls} value={outlineForm.target_level} onChange={(e) => setOutlineForm((f) => ({ ...f, target_level: e.target.value }))}>
                            <option value="beginner">Beginner</option><option value="intermediate">Intermediate</option><option value="advanced">Advanced</option>
                          </select>
                        </div>
                        <div className="col-span-2">
                          <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Learning Objectives (optional)</label>
                          <textarea className={`${inputCls} resize-none`} rows={2} value={outlineForm.learning_objectives} onChange={(e) => setOutlineForm((f) => ({ ...f, learning_objectives: e.target.value }))} placeholder="What students will be able to do after this course…" />
                        </div>
                      </div>
                      <button type="submit" disabled={aiLoading} className="w-full py-3 sc-btn-glow rounded-xl text-sm font-bold cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2">
                        {aiLoading ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Generating…</> : "Generate Outline →"}
                      </button>
                    </form>
                  )}
                    ...
                    <div ref={aiResultRef} className="space-y-3">
                      <div className="border border-[var(--border)]/40 rounded-xl overflow-hidden">
                        <div className="px-4 py-2 border-b border-[var(--border)]/30 bg-green-500/5 flex items-center justify-between">
                          <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" /><span className="text-xs text-[var(--muted)] font-semibold uppercase tracking-wider">Generated Outline</span></div>
                      ...
                      {aiError && <p className="text-red-400 text-sm">{aiError}</p>}
                      <button onClick={saveOutline} disabled={outlineSaving} className="w-full py-3 sc-btn-glow rounded-xl text-sm font-bold cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2">
                        {outlineSaving ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving…</> : "💾 Save Outline to Library"}
                      </button>
                    </div>
                  )}
                  ...
                  {savedOutlines.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-bold text-[var(--muted)] uppercase tracking-wider">Saved Outlines ({savedOutlines.length})</p>
                      {savedOutlines.map((o) => (
                        <div key={o.id} className="border border-[var(--border)]/40 rounded-xl p-3 flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <p className="text-sm font-bold text-[var(--text)] truncate">{o.courseTitle}</p>
                            <p className="text-xs text-[var(--muted)]">{o.subject} · {o.durationWeeks}w · {o.targetLevel} · {new Date(o.createdAt).toLocaleDateString()}</p>
                          </div>
                          <button onClick={() => deleteOutline(o.id)} className="text-red-400 hover:text-red-300 text-xs shrink-0 cursor-pointer">🗑</button>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {aiModal === "agent" && (
                <>
                  {!aiResult && (
                    <form onSubmit={(e) => { e.preventDefault(); runAi("/agent", { task: agentTask, context: { teacher_id: user.id } }); }} className="space-y-4">
                      <div className="p-3 glass rounded-xl border border-violet-500/20 space-y-1">
                        <p className="text-xs font-semibold text-violet-400 uppercase tracking-wider mb-2">Example Tasks</p>
                        {["Generate a 5-question quiz on recursion AND create a course outline for Web Dev","Create a complete 8-week course outline for Introduction to Machine Learning","Summarize this content then generate 5 quiz questions from it"].map((ex) => (
                          <button key={ex} type="button" onClick={() => setAgentTask(ex)} className="block w-full text-left text-xs text-[var(--muted)] hover:text-[var(--text)] px-3 py-1.5 rounded-lg hover:bg-violet-500/10 transition-all truncate">→ {ex}</button>
                        ))}
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Task *</label>
                        <textarea className={`${inputCls} resize-none`} rows={3} required value={agentTask} onChange={(e) => setAgentTask(e.target.value)} placeholder="Describe what you want the AI agent to do…" />
                      </div>
                      <button type="submit" disabled={aiLoading} className="w-full py-3 sc-btn-glow rounded-xl text-sm font-bold cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2">
                        {aiLoading ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Running Agent…</> : "Run Agent ⚡"}
                      </button>
                    </form>
                  )}
                  {aiResult && (
                      ...
                      <div className="border border-[var(--border)]/40 rounded-xl overflow-hidden">
                        <div className="px-4 py-2 border-b border-[var(--border)]/30 bg-green-500/5 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                            <span className="text-xs text-[var(--muted)] font-semibold uppercase tracking-wider">Agent Result</span>
                            {aiResult.tools_used && <div className="flex gap-1">{aiResult.tools_used.map((t,i) => <span key={i} className="text-xs px-2 py-0.5 rounded-full bg-violet-500/15 text-violet-400">{t.tool}</span>)}</div>}
                          </div>
              ...
              {aiError && !quizSaved && editableQuestions.length === 0 && (
                <div className="p-4 rounded-xl border border-red-400/40 bg-red-500/10 text-red-400 text-sm">{aiError}</div>
              )}
            </div>
          </div>
        </div>
