...
describe("Auth API", () => {
  ...
  describe("POST /api/auth/register", () => {
    ...
    it("returns 400 when required fields are missing", async () => {
      const res = await request.post("/api/auth/register").send({ email: "missing@test.io" }); // no name or password
  ...
  describe("POST /api/auth/verify-otp", () => {
    it("verifies a valid OTP, marks user verified, and sets auth cookie", async () => {
      ...
      // Step 2: verify
      const res = await request.post("/api/auth/verify-otp").send({ email: "bob@test.io", otp });
    ...
    it("returns 400 when otp field is missing", async () => {
      const res = await request.post("/api/auth/verify-otp").send({ email: "nobody@test.io" });
  ...
  describe("POST /api/auth/login", () => {
    ...
    it("returns 400 when required fields are missing", async () => {
      const res = await request.post("/api/auth/login").send({ email: "nobody@test.io" });
  ...
  describe("POST /api/auth/resend-otp", () => {
    it("resends OTP for an unverified user", async () => {
      ...
      const res = await request.post("/api/auth/resend-otp").send({ email: "grace@test.io" });
    ...
    it("returns 400 for an already-verified user", async () => {
      ...
      const res = await request.post("/api/auth/resend-otp").send({ email: "henry@test.io" });
